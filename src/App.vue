<template>
  <v-app>
    ...
    >
      <v-list>
        <template value="true" v-for="(item, i) in items" active-class="caption">
    ...
    <v-toolbar app dark color="primary" clipped-left>
      ...
      <v-spacer></v-spacer>
      <v-toolbar-items>
        <v-btn flat v-on:click.stop="editProfile()" v-if="profile!=null">{{ profile.fullName() }}</v-btn>
    ...
    <v-footer fixed app>
      <span>
        &copy; 2018
        <strong>WF and WJ McCann</strong>
      </span>
    </v-footer>
  </v-app>
</template>
      ...
      items: [
        {
          ...
          title: 'Events',
          isItem: true,
          method: () => this.showEvents()
        },
        {
          icon: 'people',
          title: 'Friends',
          isItem: true,
          method: () => this.showFriends()
        },
