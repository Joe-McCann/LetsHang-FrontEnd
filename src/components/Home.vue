<template>
  <v-container fluid>
    <v-layout column>
      <v-flex xs12 md8 offset-md2 p-0 m-0>
            ...
            <v-slide-y-transition mode="out-in">
              <v-layout column align-center>
                <v-flex xs12>
                  ...
                  <blockquote>&#8220;Hanging out with friends should be easy.&#8221;</blockquote>
                  <router-link to="./eventdetailhorizontal">
                    <div
                      class="headline mt-4"
                      @click="handleNewEvent"
                    >Let's Get Started {{ myFirstName }}!</div>
                  </router-link>
                </v-flex>
              </v-layout>
...
export default {
  ...
  data: function() {
    ...
    return {
      ...
      messageText:
        "Please confirm that you wish to decline this event. If you are the only attendee then the event will be cancelled.",
      profile: store.getters.currentUser
    };
  },
  methods: {
    resetForm: function(payload) {
      this.openForm = payload.isopen;
    },
